...
/*
 * 定义 Hiphp 框架核心类，并初始化框架基本设置
 */

/**
 * Hiphp 框架基本库所在路径s
 */
define('HICORE_PATH', dirname(__FILE__)."/");
...
if (empty($G_CLASS_FILES)) {
    require HICORE_PATH . '/config/hiphp_class_files.php';
}

/**
 * 类 Hi 是 HiPHP 框架的核心类，提供了框架运行所需的基本服务
 *
 * 类  Hi 提供 HiPHP 框架的基本服务，包括：
 *
 * -  设置的读取和修改；
 * -  类定义文件的搜索和载入；
 * -  对象的单子模式实现，以及对象注册和检索；
...
class Hi {
     ...
     * @var array
     */
    private static $_model_class_path = array();
    /**
     * model类搜索路径的选项
     *
     * @var array
     */
    private static $_model_class_path_options = array();




    /* controller类搜索路径 */
    private static $_controller_class_path = array();
    /**
     * controller类搜索路径的选项
     *
     * @var array
     */
    private static $_controller_class_path_options = array();
    /**
     * 类搜索路径
     *
     * @var array
    ...
    private static $_deffaultdb = '';

    /**
     * 返回 HiPHP 版本号
     *
     * @return string HiPHP 版本号
     */
    static function version() {
        return '0.1';
    }

    static function addDb($db, $dbkey, $isdeffault=false) {
        if (!isset(self::$_db[$dbkey])) {
            self::$_db[$dbkey] = $db;
        }
        if (isdeffault) {
            self::$_deffaultdb = $dbkey;
        }
    }

    static function getDb($dbkey=null) {
        if (empty($dbkey))
            $dbkey = self::$_deffaultdb;
        if (isset(self::$_db[$dbkey])) {
            return self::$_db[$dbkey];
        } else {
            throw new HiException('database ' . $dbkey . ' not set ');
        }
    }
    
    private static $_sharddb_mlink = array();
    
    static function addSharddb_mlink($mlink) {
    	if(!in_array($mlink,self::$_sharddb_mlink)){
    		self::$_sharddb_mlink[]=$mlink;    		
    	}    	
    }
    
    static function closeSharddb_mlink(){   
    	//var_dump(self::$_sharddb_mlink); exit;
    	foreach (self::$_sharddb_mlink as $mlink) {
    		mysql_close($mlink);
    	}
    }
     ...
     * echo Hi::ini('option_group/my_option');
     * @endcode
     *
     ...
     * @code php
     * // 修改一个设置项
     * Hi::changeIni('option_group/my_option2', 'new value');
     * @endcode
     *
     ...
     * @endcode
     *
     * 上述代码展示了 Hi::changeIni() 的一个重要特性：保持已有设置的层次结构。
     *
     * 因此如果要完全替换某个设置项和其子项目，应该使用 Hi::replaceIni() 方法。
     *
     ...
     * 替换已有的设置值
     *
     * Hi::replaceIni() 表面上看和 Hi::changeIni() 类似。
     * 但是 Hi::replaceIni() 不会保持已有设置的层次结构，
     * 而是直接替换到指定的设置项及其子项目。
     *
     * @code php
     * // 假设已有的设置为
     ...
     * @endcode
     *
     * 从上述代码的执行结果可以看出 Hi::replaceIni() 和 Hi::changeIni() 的重要区别。
     *
     * 不过由于 Hi::replaceIni() 速度比 Hi::changeIni() 快很多，
     * 因此应该尽量使用 Hi::replaceIni() 来代替 Hi::changeIni()。
     *
     * @param string|array $option 要修改的设置项名称，或包含多个设置项目的数组
     * @param mixed $data 指定设置项的新值
     */
     ...
     *
     * @code php
     * Hi::loadClass('Table_Posts', array('/www/mysite/app', '/www/mysite/lib'));
     * @endcode
     *
    ...
    static function loadClass($class_name, $dirs = null, $throw = true) {
        ...
        if (isset($G_CLASS_FILES[$class_name])) {
            require HICORE_PATH . DS . $G_CLASS_FILES[$class_name];
            return $class_name;
        }
        if (preg_match('/Model/i', $class_name) && $class_name!='ModelController' && !empty(self::$_model_class_path)) {
            $filename =  $class_name . '.php';//self::$_model_class_path . DS .
            return self::loadClassFile($filename, self::$_model_class_path, $class_name, "", $throw);
            /*if (is_file($path)) {
                require $path;
                return $class_name;
            }*/
        }
        if (preg_match('/Controller/i', $class_name) && !empty(self::$_controller_class_path)) {
            $filename =  $class_name . '.php';//self::$_model_class_path . DS .
            return self::loadClassFile($filename, self::$_controller_class_path, $class_name, "", $throw);

            /*$path = self::$_controller_class_path . DS . $class_name . '.php';
            if (is_file($path)) {
                require $path;
                return $class_name;
            }*/
     ...
     * 添加一个类搜索路径
     *
     * 如果要使用 Hi::loadClass() 载入非 HiPHP 的类，需要通过 Hi::import() 添加类类搜索路径。
     *
     * 要注意，Hi::import() 添加的路径和类名称有关系。
     *
     * 例如类的名称为 Vendor_Smarty_Adapter，那么该类的定义文件存储结构就是 vendor/smarty/adapter.php。
     * 因此在用Hi::import() 添加 Vendor_Smarty_Adapter 类的搜索路径时，
     * 只能添加 vendor/smarty/adapter.php 的父目录。
     *
     * @code php
     * Hi::import('/www/app');
     * Hi::loadClass('Vendor_Smarty_Adapter');
     * // 实际载入的文件是 /www/app/vendor/smarty/adapter.php
     * @endcode
     *
     * 由于 HiPHP 的规范是文件名全小写，因此要载入文件名存在大小写区分的第三方库时，
     * 应该指定 import() 方法的第二个参数。
     *
     * @code php
     * Hi::import('/www/app/vendor');
     * Hi::loadClass('Zend_Mail');
     ...
     */
    static function import($dir, $case_sensitive = true) {
    ...
    static function importmodel($dir, $case_sensitive = true) {
        $real_dir = realpath($dir);
        if ($real_dir) {
            $dir = rtrim($real_dir, '/\\');
            if (!isset(self::$_model_class_path[$dir])) {
                self::$_model_class_path[$dir] = $dir;
                self::$_model_class_path_options[$dir] = $case_sensitive;
            }
        }
    }

    static function importcontroller($dir, $case_sensitive = true) {
        $real_dir = realpath($dir);
        if ($real_dir) {
            $dir = rtrim($real_dir, '/\\');
            if (!isset(self::$_controller_class_path[$dir])) {
                self::$_controller_class_path[$dir] = $dir;
                self::$_controller_class_path_options[$dir] = $case_sensitive;
            }
        }
    }
     ...
     *
     * @code php
     * Hi::loadClassFile('Smarty.class.php', $dirs, 'Smarty');
     * @endcode
     *
    ...
    static function loadClassFile($filename, $dirs, $class_name, $dirname = '', $throw = true) {
        ...
        if (!class_exists($class_name, false) && !interface_exists($class_name, false)) {
            if ($throw) {
                throw new Hi_ClassNotDefinedException($class_name, $path);
            }
            return false;
        }
        return $class_name;
     ...
     *
     * 与 PHP 内置的 require 和 include 相比，Hi::loadFile() 会多处下列特征：
     *
     * <ul>
    ...
    static function loadFile($filename, $dirs = null, $throw = true) {
        if (preg_match('/[^a-z0-9\-_.]/i', $filename)) {
            throw new Hi_IllegalFilenameException($filename);
        }
        ...
        if ($throw)
            throw new Hi_FileNotFoundException($filename);
        return false;
    }
     ...
     * </ul>
     *
     * 使用 Hi::singleton() 的好处在于多次使用同一个对象时不需要反复构造对象。
     *
     * @code php
     * // 在位置 A 处使用对象 My_Object
     * $obj = Hi::singleton('My_Object');
     * ...
     * ...
     * // 在位置 B 处使用对象 My_Object
     * $obj2 = Hi::singleton('My_Object');
     ...
     */
    static function singleton($class_name, $param1 = null) {
        ...
        if (is_null($param1)) {
        	...
        	return self::register(new $class_name($param1), $class_name);
        }
    }
     ...
     * 以特定名字在对象注册表中登记一个对象
     *
     * 开发者可以将一个对象登记到对象注册表中，以便在应用程序其他位置使用 Hi::registry() 来查询该对象。
     * 登记时，如果没有为对象指定一个名字，则以对象的类名称作为登记名。
     *
     * @code php
     * // 注册一个对象
     * Hi::register(new MyObject());
     * .....
     * // 稍后取出对象
     * $obj = Hi::regitry('MyObject');
     * @endcode
     *
     * 当 $persistent 参数为 true 时，对象将被放入持久存储区。
     * 在下一次执行脚本时，可以通过 Hi::registry() 取出放入持久存储区的对象，并且无需重新构造对象。
     *
     ...
     *
     * @code php
     * if (!Hi::isRegistered('MyObject'))
     * {
     *      Hi::registry(new MyObject(), 'MyObject', true);
     * }
     * $app = Hi::registry('MyObject');
     * @endcode
     *
    ...
    static function register($obj, $name = null, $persistent = false) {
        if (!is_object($obj)) {
            ...
            throw new HiException('Type mismatch. $obj expected is object, actual is "%s".',
                    gettype($obj));
        }
    ...
    static function registry($name) {
        ...
        throw new HiException('No object is registered of name "%s".', $name);
    }
     ...
     *
     * Hi::cache() 可以指定 $policy 参数来覆盖缓存数据本身带有的策略。
     * 具体哪些策略可以使用，请参考不同缓存服务的文档。
     *
     * $backend_class 用于指定要使用的缓存服务对象类名称。例如 QCache_File、QCache_APC 等。
     *
     * @code php
     * $data = Hi::cache($cache_id);
     * if ($data === false)
     * {
     *     $data = ....
     *     Hi::writeCache($cache_id, $data);
     * }
     * @endcode
     *
    ...
    static function cache($id, array $policy = null, $backend_class = null) {
        static $obj = null;
        $cache_backend = is_null($backend_class) ? self::ini('runtime_cache_backend') : $backend_class;
        $servers = Hi::ini("runtime_cache_policy/$cache_backend/servers");
        if (count($servers)) {
        	# 配置文件设置了自定义缓存服务器
        	$obj = self::singleton($cache_backend, array('servers' => $servers));
        } else {
        	# 调用cache类里面的默认缓存服务器
        	$obj = self::singleton($cache_backend);
        }
     ...
     */
    static function registerAutoload($class = 'Hi', $enabled = true) {
        if (!function_exists('spl_autoload_register')) {
            require_once HICORE_PATH . '/hicore/HiException.php';
            throw new HiException('spl_autoload does not exist in this PHP installation');
        }
...
Hi::registerAutoload();
include HICORE_PATH . '/hicore/common.php'; //加载公共函数库
